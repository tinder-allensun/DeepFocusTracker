...
- Tests: **Swift Testing** (`DeepFocusTrackerTests/`), run in GitHub Actions CI.
...
| M3.3 — Test suite & CI | ✅ shipped | Swift Testing target over the whole core + GitHub Actions guardrail |
...
### M3.3 — Test suite & CI ✅ (shipped 2026-07-20)
- **Goal:** lock in the core's behavior so future changes are safe — a guardrail against regressions, not just a green build.
- **Built:**
  - A **Swift Testing** target (`DeepFocusTrackerTests/`, 45 tests / 8 suites): the pure aggregators (`UsageAggregator`, `InsightsService`, `TimeFormat`, `LabelChooser`) plus SwiftData integration tests for `FocusController` (lifecycle, label upsert, seeding, open-block recovery), `Rollups` (accumulate/decrement, row cleanup), and `SessionHistory` (delete session + intervals, no orphans) — including a `DayRollup`/`DayAppRollup`-vs-raw consistency check.
...
- **Key files:** `DeepFocusTrackerTests/*`, `.github/workflows/ci.yml`; plus a `DFT_TESTING` bare-host guard in `DeepFocusTrackerApp` so the hosted test bundle doesn't stand up a second SwiftData container (which would trap CoreData).
- **Verified:** `xcodebuild test` green (45 tests); production build clean.
...
- **Testing:** the pure aggregators (`UsageAggregator`, `InsightsService`, `LabelChooser`, `TimeFormat`) are kept separate from UI so they stay unit-testable — and as of M3.3 there's a **Swift Testing** target (`DeepFocusTrackerTests/`) covering them plus the SwiftData paths, run in CI. Every change is expected to add/keep tests green (see [CLAUDE.md](CLAUDE.md#testing-the-guardrail)).
